...
switch ($mod_tip)
{
	case "add":
		...
		//tomo las fechas de expiración para cada portal
		...
		//tomo la cantidades máximas permitidas por portal
		...
		//ahora hago la verificación
		//si la suma de propiedades actuales supera el máximo y el numero máximo no es cero(ilimitados)
		if($mos_por_1=="on"){
			...
			}else if($fec_exp1 < $fec_act && $fec_exp1!=-1){ //o si la fecha de expiración es mayor que la actual y no esta en cero(ilimitado)
				//no se puede mostrar en el portal 1
				$mos_por_1=0;
		...
		if($mos_por_2=="on"){
			...
			}else if($fec_exp2 < $fec_act && $fec_exp2!=-1){ //o si la fecha de expiración es mayor que la actual y no esta en cero(ilimitado)
				//no se puede mostrar en el portal 2
				$mos_por_2=0;
		...
		if($mos_por_3=="on"){
			...
			}else if($fec_exp3 < $fec_act && $fec_exp3!=-1){ //o si la fecha de expiración es mayor que la actual y no esta en cero(ilimitado)
				//no se puede mostrar en el portal 3
				$mos_por_3=0;
		...
		if($mos_por_4=="on"){
			if($cant_por_4 > $max_prp4 && $max_prp4!=0){
				...
				$mos_por_4=0;	
			}else if($fec_exp4 < $fec_act && $fec_exp4!=-1){ //o si la fecha de expiración es mayor que la actual y no esta en cero(ilimitado)
				//no se puede mostrar en el portal 4
				$mos_por_4=0;
		...
		if (mysql_num_rows($verif))	{
			...
			$max_nov=mysql_result(mysql_query("select max(nov_id) from novedades"),0,0);
				...
				$message="Se dió de alta una nueva propiedad en $condicion, ID $max_id, ubicada en $domicilio. ";				
				
				$ins_novedades=mysql_query("insert into novedades values($max_nov,'$message','24',CURDATE(),'',CURTIME());");
				
				$ins_nov_x_emp=mysql_query("insert into nov_x_emp values($max_nov,'49',0);");//Matias Cocucci
	...
	***********************************************************************************************************************/
	case "del":
		...
		for ($i=0;$i<count($arr_estados)-1;$i++)
		{
			...
			{
				$res=mysql_fetch_array(mysql_query("select loc_id,pro_id,con_id,prp_dom,prp_bar,prp_cartel from propiedades where usr_id=$usr_id and prp_id=$prp_id"));
	...
	case "edit":
		...
		for ($i=1;$i<=15;$i++)
		{
			...
			else if($$foto_id!="")
			{
					$modif_fotos=1;
					$nueva=$id_foto.".gif";
					list($size_x,$size_y,$tipo,$atr)=getimagesize($$foto_id);
			 		if(filesize($$foto_id)<$MAX_TAM_FOT || ($size_x<600 && $size_y<600) )
			 		{
						copy($$foto_id,"../fotos/".$id_foto.".gif");
					}else{	
						redim_foto($foto_id,"../fotos/",$id_foto);
