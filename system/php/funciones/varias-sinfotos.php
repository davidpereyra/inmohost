	...
	if($dem_id){
		...
		if(mysql_num_rows($result)){
			...
			print "
				<script language='javascript'>
						parent.Dialog.alert(\"<div style='font-size:12px;'><br>Estimado <b>".$_SESSION['usr_login']."</b>, Ud. acaba de agregar o modificar una demanda que coincide con una o más propiedades.<br><br>A través del siguiente listado Ud. podrá comunicarse por telefono con el interesado o hacer click en<br>ENVIAR MAIL y el sistema enviará automaticamente un aviso al interesado del inmueble en cartera.<br></div><br> $string  \", {windowParameters: {className:\"alert\", width:700, height:400}, 				
						okLabel: \"Cerrar\"
						 });
					hideToolTip();
				</script>
		...
		while ($fila=mysql_fetch_array($res)){
			...
			//	print $filt_loc;
				...
				//print "<Br> select $regs from $tablas where $filtro <br>";		
		...
		if($string){  
			...
			print "
				<script language='javascript'>
						parent.Dialog.alert(\"<div style='font-size:12px;'><br>Estimado <b>".$_SESSION['usr_login']."</b>, Ud. acaba de agregar o modificar un inmueble que coincide con una o más demandas pendientes.<br><br>A través del siguiente listado Ud. podrá comunicarse por telefono con el interesado o hacer click en<br>ENVIAR MAIL y el sistema enviará automaticamente un aviso al interesado del inmueble en cartera.<br></div><br> $string \", {windowParameters: {className:\"alert\", width:700, height:400}, 
						okLabel: \"Cerrar\"
						 });
					hideToolTip();
				</script>
	...
	function redim_foto($foto,$dest,$nom_foto){
		...
		if ($fo->uploaded) {
		  ...
		  if ($fo->processed) {
		    ...
		    echo 'Error al achicar la foto: ' . $fo->error;
		  }
				...
				*/
				...
				$str.="
						...
						?>
							...
							<script language=\"javascript\">
									location.href=\"http://<?print \$ip?>/inmohostV2.0/system/actualizador_pre.php?usr_id=<?print \$usr_id?>&borrar_cache_bar_priv=1\";
							</script>
	...
	//				print $cadena."<br>";
				...
				$str.="
						...
						?>
							...
							<script language=\"javascript\">
									location.href=\"http://<?print \$ip?>/inmohostV2.0/system/actualizador_pre.php?usr_id=$usr_id&borrar_cache_usuario=1\";
							</script>
...
function armar_script($usr_id){
	...
	//				print $cadena."<br>";
							...
							case "modificacion":
								...
								if(mysql_num_rows($res1)){
									...
									while ($fila_ser=mysql_fetch_array($res_alt)){
											...
											$cadena="insert into ser_x_prp_ihost values('$fila[prp_id]',$fila_ser[ser_id],'$fila_ser[valor]',$fila[usr_id])";							
											$str.="mysql_query(\"$cadena\") or \$error=\"Consulta: $cadena\".mysql_error();\r\n";	
										/*	$str.="mysql_query(\"$cadena\") or die(\"Consulta:$cadena NRO $j\".mysql_error());\r\n";	
											$j++;*/
									}
				...
				$str.="
						...
						?>
							<script language=\"javascript\">
									location.href=\"../act_inmohost.php?usr_id=<?print \$usr_id?>&ip=<?print \$ip?>\";
							</script>
