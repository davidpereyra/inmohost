 ...
 *   - added {@link file_auto_rename} automatic file renaming if the same filename already exists</li>
 *   - added {@link file_safe_name} safe formatting of the filename (spaces to _underscores so far)</li>
...
class upload {
    ...
    function init() {
        ...
        $this->file_max_size           = 4097152;  // LIMITE DE PROTECCION

        $this->image_resize            = false;    // resize the image
        $this->image_convert           = '';       // convert. values :''; 'png'; 'jpeg'; 'gif'

        $this->image_x                 = 800;		// LIMITE DE PROTECCION
        $this->image_y                 = 800;		// LIMITE DE PROTECCION
    ...
    function upload($file) {
        ...
        if ($this->uploaded) {
            ...
            if (!array_key_exists($this->file_src_mime, $this->allowed)) {
                $this->uploaded = false;
                $this->error = 'Tipo de archivo no admitido. '.$this->file_src_name_ext;
            }
            
        }
        if ($this->uploaded) {
            $this->log .= '- file size OK<br />';
            $this->log .= '- file mime OK<br />';
        }
    ...
    function process($server_path) {
        ...
        if ($this->uploaded) {
            ...
            if ($this->file_safe_name) { // formats the name
                ...
                $mal = array("á", "Á", "é", "É", "í", "Í", "ó", "Ó", "ú", "Ú", "ñ", "Ñ", "%", "&", "$", "!", "¡", "(", ")", "'", "?", "¿", "}", "{", "*", ",", ";", ":", " ");
				$bien   = array("a", "A", "e", "E", "i", "I", "o", "O", "u", "U", "n", "N", "_", "AND", "S", "_", "_", "_", "_", "_", "_", "_", "_", "_", "_", "_", "_", "_", "_");

				$this->file_dst_name_body = str_replace($mal, $bien, $this->file_dst_name_body) ;
                        ...
                        switch($this->image_convert) {
                            ...
                            case 'jpg':
                                if (!function_exists('imagejpeg')) {
                                    ...
                                    //$result = @imagejpeg ($image_dst, "3-886-2.jpg", $this->jpeg_quality);
                                	$result = imagejpeg ($image_dst, $this->file_dst_pathname, $this->jpeg_quality);
                                    if (!$result) {
                                        $this->processed = false;
                                        $this->error = 'Impossible to create file' . $image_dst .  $this->file_dst_pathname;
                                    } else {
