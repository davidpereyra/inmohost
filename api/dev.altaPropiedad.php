...
include_once 'objects/dev.propiedad.php';
...
$propiedad->prp_referente=$data->prp_referente;
...
$propiedad->valor7=$data->valor7;//Cochera
...
if( !empty($propiedad->tip_id) &&
    !empty($propiedad->prp_dom) 
){

    $propiedad->altaPrpInmohost();

    if( $propiedad->errores=="" ){

        http_response_code(201);

          echo json_encode(array("message" => "La nueva propiedad ha sido guardada. ID: ".$propiedad->prp_id));
    
    }else{

        http_response_code(503);

        echo json_encode(array("message" => "Fallo al intentar crear: ".$propiedad->errores));
    }
    ...
    echo json_encode(array("message" => "Faltan completar datos minimos requeridos. Tipo: ".$propiedad->tip_id." - Domicilio: ".$propiedad->prp_dom));
}
